...
io.on('connection', (socket) => {
  ...
  socket.on('join_room', async (roomId: string, nickname: string = 'Guest', passcode?: string) => {
    ...
    try {
      ...
      if (!room) {
        room = await RoomModel.create({ roomId: cleanRoomId, strokes: [] });
      }
      ...
      // Assign host if this is the first user joining the room
  ...
  socket.on('selection:update', (payload: { roomId: string; shapeId: string | null }) => {
    // We won't use strict validation here to keep it simple, but we should verify the user
    const user = users.get(socket.id);
    if (user && user.roomId === payload.roomId) {
      socket.to(payload.roomId).emit('selection:update', {
        socketId: socket.id,
